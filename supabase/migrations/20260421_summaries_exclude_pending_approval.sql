-- ============================================================================
-- Migration: Exclude pending_approval records from time-summary views
-- Date: 2026-04-21
--
-- Builds on 20260417_exclude_idle_from_summaries.sql. Follows the same
-- DROP + recreate idiom (CREATE OR REPLACE cannot change column types on
-- views). Only one WHERE clause is added to the activity_records subquery
-- in each of the three summary views:
--
--   AND COALESCE(act.approval_status, 'approved') <> 'pending_approval'
--
-- Historical rows have approval_status = NULL. COALESCE converts NULL
-- to 'approved', so those rows stay visible. Only new AI-assigned records
-- awaiting human review are hidden from summaries -- consistent with the
-- worklog-sync block introduced in the same release.
-- ============================================================================
...
FROM (
    ...
    --   - excludes idle and lock-screen records (from 20260417)
    --   - excludes pending_approval records (this migration)
    SELECT
        act.user_id,
        act.organization_id,
        u.display_name AS user_display_name,
...
-- ============================================================================
-- Preserve SECURITY INVOKER setting (same as 20260417)
-- ============================================================================
...
COMMENT ON VIEW public.daily_time_summary IS
  'Daily time aggregation combining legacy screenshot analysis and activity records. Excludes idle/lock-screen time and records awaiting human approval. SECURITY INVOKER.';
COMMENT ON VIEW public.weekly_time_summary IS
  'Weekly time aggregation combining legacy screenshot analysis and activity records. Excludes idle/lock-screen time and records awaiting human approval. SECURITY INVOKER.';
COMMENT ON VIEW public.monthly_time_summary IS
  'Monthly time aggregation combining legacy screenshot analysis and activity records. Excludes idle/lock-screen time and records awaiting human approval. SECURITY INVOKER.';
