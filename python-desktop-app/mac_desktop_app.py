...
class AtlassianAuthManager:
    ...
    def get_supabase_config(self):
        ...
        try:
            ...
            # Debug: Show token info for troubleshooting (first 20 chars only for security)
            print(f"[DEBUG] Using access token: {access_token[:20]}...")
            print(f"[DEBUG] AI Server URL: {ai_server_url}")
            ...
            print(f"[DEBUG] AI Server response status: {response.status_code}")
            ...
            if response.status_code != 200:
                try:
                    ...
                    print(f"[ERROR] Failed to get Supabase config: HTTP {response.status_code}")
            ...
            if not result.get('success'):
                ...
                print(f"[ERROR] Failed to get Supabase config: {error_msg}")
            ...
            print("[OK] Supabase configuration loaded from AI server")
            return True
...
class TimeTracker:
    ...
    def initialize_supabase(self):
        ...
        # First try to get config from AI server (production flow)
        print("[INFO] Fetching Supabase configuration from AI server...")
        config_from_ai_server = self.auth_manager.get_supabase_config()
        
        if not config_from_ai_server:
            print("[WARN] AI server config failed, trying development fallback...")
            
            # Development fallback: Check if environment variables are available
            dev_url = os.getenv('SUPABASE_URL')
            dev_anon = os.getenv('SUPABASE_ANON_KEY') 
            dev_service = os.getenv('SUPABASE_SERVICE_ROLE_KEY')
            
            if dev_url and dev_anon:
                print("[INFO] Using development Supabase config from environment variables")
                set_runtime_supabase_config(dev_url, dev_anon, dev_service)
            else:
                print("[ERROR] No Supabase config available (AI server failed, no env vars)")
                print("[INFO] Create a .env file with SUPABASE_URL, SUPABASE_ANON_KEY, SUPABASE_SERVICE_ROLE_KEY")
                return False
        ...
        try:
            ...
            # Initialize Supabase clients (using default timeout settings for compatibility)
            ...
            source = "AI server" if config_from_ai_server else "environment variables"
            self.add_admin_log('INFO', f'Supabase initialized from {source}')
            return True
    ...
    def register_organization(self):
        ...
        try:
            ...
            if not client:
                print("[WARN] Cannot register organization: Supabase client not available")
                print("[INFO] Please authenticate via web interface at http://localhost:{}".format(self.web_port))
                return None
    ...
    def fetch_project_settings(self, force_refresh=False):
        ...
        try:
            ...
            client = self.supabase_service if self.supabase_service else self.supabase
            if not client:
                print("[WARN] Cannot fetch project settings: Supabase client not available")
                print("[INFO] Please authenticate via web interface at http://localhost:{}".format(self.web_port))
    ...
    def fetch_tracking_settings(self):
        ...
        # Only fetch if Supabase is initialized and user is authenticated
        if not self.supabase or not self.supabase_initialized:
            print("[INFO] Supabase not initialized - using default tracking settings")
            return
        
        if not self.current_user_id:
            print("[INFO] No user authenticated - using default tracking settings")
            return
        ...
        try:
            ...
            print("[INFO] Using default tracking settings")
    ...
    def fetch_notification_settings(self):
        ...
        try:
            ...
            print("[INFO] Using default notification settings")
            # Continue with default settings
    ...
    def get_unassigned_work_summary(self):
        ...
        try:
            ...
            print("[INFO] Will retry when authenticated and connected")
            return None
    ...
    def check_and_notify_unassigned_work(self):
        ...
        # Only check if authenticated and Supabase is initialized
        if not self.current_user or not self.supabase_initialized:
            print("[INFO] Skipping unassigned work check - not authenticated or Supabase not initialized")
            return
    ...
    def upload_screenshot(self, screenshot, window_info, use_previous_window=False):
        ...
        if not storage_client:
            print("[WARN] Cannot upload screenshot: Supabase storage client not available")
            print("[INFO] Please authenticate via web interface at http://localhost:{}".format(self.web_port))
            print("[INFO] Screenshot will be saved offline for later sync")
            # Save offline and return early
            return self._save_screenshot_offline_fallback(screenshot, window_info)
        ...
        try:
            ...
            # Provide specific guidance for DNS resolution errors
            if "nodename nor servname provided" in error_msg or "NXDOMAIN" in error_msg:
                print("[ERROR] DNS resolution failed - check Supabase URL configuration")
                print(f"[ERROR] Current URL: {get_env_var('SUPABASE_URL')}")
                print("[ERROR] This usually means:")
                print("[ERROR]   1. Supabase project URL is incorrect or outdated")
                print("[ERROR]   2. DNS server issues on your network")  
                print("[ERROR]   3. Corporate firewall blocking domain resolution")
                self.add_admin_log('ERROR', f'DNS resolution failed for Supabase URL')
            elif "timeout" in error_msg.lower():
                print("[ERROR] Connection timeout - network may be slow or unstable")
                self.add_admin_log('ERROR', 'Connection timeout during upload')
    ...
    def _save_screenshot_offline_fallback(self, screenshot, window_info, error_context=None):
        """Save screenshot offline as fallback when upload fails or client unavailable
        ...
        Returns:
            str: Offline record identifier or None if failed
        """
        try:
            print("[INFO] Attempting to save screenshot offline as fallback...")
            if error_context:
                print(f"[INFO] Fallback reason: {error_context}")
            
            # Convert screenshot to bytes
            img_buffer = BytesIO()
            
            # Resize if needed (same logic as main upload)
            max_dimension = 1920
            if max(screenshot.size) > max_dimension:
                ...
                print(f"[DEBUG] Resized screenshot for offline: {screenshot.size}")
            ...
            if screenshot.mode in ('RGBA', 'LA'):
                ...
                rgb_image.paste(screenshot, mask=screenshot.split()[-1] if screenshot.mode == 'RGBA' else None)
            ...
            try:
                ...
                print(f"[WARN] Failed to create thumbnail for offline save: {e}")
            ...
            screenshot_data = {
                ...
                'organization_id': self.organization_id,
                'timestamp': timestamp.isoformat(),
                'storage_path': f"{self.current_user_id or 'anonymous'}/screenshot_{int(timestamp.timestamp())}.jpg",
                'window_title': window_info.get('title', 'Unknown'),
                'application_name': window_info.get('app', 'Unknown'),
                'file_size_bytes': len(img_bytes),
                'start_time': start_time.isoformat(),
            ...
            if local_id:
                ...
                print("[ERROR] Failed to save screenshot to offline storage")
                return None
                
        except Exception as e:
            print(f"[ERROR] Offline screenshot save failed: {e}")
            return None
    ...
    def sync_offline_data(self, force=False):
        ...
        if not db_client or not storage_client:
            print("[WARN] Cannot sync offline data: Supabase clients not available")
            print("[INFO] Please authenticate via web interface at http://localhost:{}".format(self.web_port))
            return None
    ...
    def tracking_loop(self):
        ...
        # Fetch initial tracking settings from Supabase (only if authenticated)
        if self.current_user and self.supabase_initialized:
            self.fetch_tracking_settings()
        else:
            print("[INFO] Using default tracking settings - not authenticated or Supabase not initialized")
        ...
        while self.running:
            try:
                ...
                if not self.is_idle:
                    # Periodically refresh tracking settings from Supabase (only if authenticated)
                    ...
                    # Periodically check for unassigned work and send notifications (only if authenticated)
        ...
        if self.auth_manager.is_authenticated():
            if is_online:
                ...
                if user_info:
                    ...
                    try:
                        # Initialize Supabase clients (fetches config from AI server)
                        print(f"[DEBUG] Attempting to initialize Supabase for user: {user_info.get('email')}")
                        print(f"[DEBUG] User account_id: {user_info.get('account_id')}")
                        print(f"[DEBUG] Current organization_id: {getattr(self, 'organization_id', 'Not set')}")
                        
                        if not self.initialize_supabase():
                            print("[WARN] Could not initialize Supabase, continuing in offline mode")
                            # Try to load cached user_id from local storage
                            self.current_user_id = self._load_cached_user_id()
                            if not self.current_user_id:
                                # Generate a temporary user ID for offline tracking
                                self.current_user_id = f"offline_{user_info.get('account_id', secrets.token_hex(8))}"
                                print(f"[INFO] Generated offline user ID: {self.current_user_id}")
                        else:
                            self.current_user_id = self.ensure_user_exists(user_info)
                            # Associate any anonymous offline records with this user
                            self._associate_offline_records()
                        ...
                        print("[INFO] Continuing in offline mode")
                        # Try to load cached user_id from local storage
                        self.current_user_id = self._load_cached_user_id()
                        if not self.current_user_id:
                            # Generate a temporary user ID for offline tracking
                    ...
                    if self.supabase_initialized:
                        ...
                        print("[INFO] Running in offline mode - screenshots will be saved locally")
                else:
